...
DECLARE_DELEGATE_RetVal(FGitSourceControlDevWorkerRef, FGetGitSourceControlDevWorker)

class FGitSourceControlDevProvider : public ISourceControlProvider
{
public:
	/** Constructor */
	FGitSourceControlDevProvider() 
		: bGitAvailable(false)
	...
	TSharedRef<FGitSourceControlDevState, ESPMode::ThreadSafe> GetStateInternal(const FString& Filename);

	/**
	 * Register a worker with the provider.
	 * This is used internally so the provider can maintain a map of all available operations.
	 */
	void RegisterWorker( const FName& InName, const FGetGitSourceControlDevWorker& InDelegate );
...
private:
	...
	TSharedPtr<class IGitSourceControlDevWorker, ESPMode::ThreadSafe> CreateWorker(const FName& InOperationName) const;

	/** Helper function for running command synchronously. */
	ECommandResult::Type ExecuteSynchronousCommand(class FGitSourceControlDevCommand& InCommand, const FText& Task);
	/** Issue a command asynchronously if possible. */
	ECommandResult::Type IssueCommand(class FGitSourceControlDevCommand& InCommand);

	/** Output any messages this command holds */
	void OutputCommandMessages(const class FGitSourceControlDevCommand& InCommand) const;
	...
	/** State cache */
	TMap<FString, TSharedRef<class FGitSourceControlDevState, ESPMode::ThreadSafe> > StateCache;

	/** The currently registered source control operations */
	TMap<FName, FGetGitSourceControlDevWorker> WorkersMap;

	/** Queue for commands given by the main thread */
	TArray < FGitSourceControlDevCommand* > CommandQueue;
