...
bool FGitCheckInWorker::Execute(FGitSourceControlCommand& InCommand)
{
	...
	if(CommitMsgFile.GetFilename().Len() > 0)
	{
		...
		if(InCommand.bCommandSuccessful)
		{
			...
			{
				InCommand.bUsingGitLfsLocking = GitSourceControlUtils::RunCommand(TEXT("push origin HEAD"), InCommand.PathToGitBinary, InCommand.PathToRepositoryRoot, TArray<FString>(), TArray<FString>(), InCommand.InfoMessages, InCommand.ErrorMessages);
				if (InCommand.bUsingGitLfsLocking)
				{
